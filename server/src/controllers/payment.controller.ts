...
import { AuthenticatedRequest, isAuth } from "../middlewares/isAuth.middleware.js";
...
export const checkOut = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    const order = await instance.orders.create(
        options as Parameters<typeof instance.orders.create>[0]
    );
...
export const paymentVerification = TryCatch(async (req: AuthenticatedRequest, res) => {
    ...
    const expectedSignature = crypto.createHmac("sha256", env.RAZORPAY_KEY_SECRET as string).update(body).digest("hex");
