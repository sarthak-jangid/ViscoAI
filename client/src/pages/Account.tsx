...
function Account() {
  ...
  console.log(user.freeRequestUsed);
  const freeLeft = Math.max(0, 3 - (user?.freeRequestUsed ?? 0));
  return (
    <div className="bg-page flex items-start justify-center px-4 pt-28 pd-12">
      <div className="w-full max-w-xl flex flex-col gap-5">
        <div className="glass-card p-6 flex items-center gap-4">
          ...
          <button
            className="feature-pill gap-2 text-red-400 border-red-500/20 hover:bg-red-500/10 transition-color cursor-pointer"
            onClick={LogoutUser}
          >
            {" "}
        ...
        <div
          className={`glass-card p-6 flex items-center gap-4 ${isPro ? "border-emerald-500/25" : "border-white/8"} `}
        >
          <div
            className={`w-12 h-12 rounded-xl flex items-center justify-center shrink-0 ${isPro ? "bg-emerald-500/15" : "bg-white/5"}`}
          >
            {isPro ? (
          ...
          <div className="glass-card p-6 flex flex-col gap-3 ">
            ...
            <div className="w-full h-1.5 bg-white/8 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width: `${((user?.freeRequestUsed ?? 0) / 3) * 100}%`,
                }}
              ></div>
              {freeLeft === 0 && (
